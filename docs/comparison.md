...
pyNebulosa, by contrast, uses gene expression values as weights in the KDE. This
...
pyNebulosa focuses on **visualization**: it produces density-weighted embedding
plots that reveal expression patterns obscured by sparsity. While pyUCell
smooths scores over a graph, pyNebulosa smooths expression over the 2D embedding
